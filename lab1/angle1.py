...
for i in range(int(rng)):
  ...
  plb.ylim([-1,1])
  plb.grid()
  plb.savefig('image_'+str(cnt)+'.jpg')
  cnt+=1


arr_of_images = np.arange(1,cnt)
def save2Gif(rang):
    im = Image.open("image_0.jpg")
    images=[]
    for i in rang:
        if i!=0:
            fpath = 'image_'+str(i)+'.jpg'
            images.append(Image.open(fpath))
    im.save('УголокCFL1.gif', save_all=True, append_images=images,loop=100,duration=1)

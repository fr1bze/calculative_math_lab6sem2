...
for i in range(int(rng)):
  ...
  plb.ylim([-1,1])
  plb.grid()
  plb.savefig('imagelv__'+str(cnt)+'.jpg')
  cnt+=1


arr_of_images = np.arange(1,cnt)
def save2Gif(rang):
    im = Image.open("imagelv__0.jpg")
    images=[]
    for i in rang:
        if i!=0:
            fpath = 'imagelv__'+str(i)+'.jpg'
            images.append(Image.open(fpath))
    im.save('Lax-Wendroff101.gif', save_all=True, append_images=images,loop=100,duration=1)
